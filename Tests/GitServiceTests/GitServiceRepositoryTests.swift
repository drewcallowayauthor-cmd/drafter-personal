...
struct GitServiceRepositoryTests {
    ...
    func logWithPathIncludesFollowAndFilter() async throws {
        ...
        #expect(invocations.first?.arguments == [
            "log", "--follow", "--format=%H%x1f%at%x1f%s%x1f%an", "--", "Manuscript/01 Arrival/01 Triage.md"
        ])
    }
    ...
    func logWithoutPathOmitsFollow() async throws {
        ...
        let invocations = await runner.invocations
        #expect(invocations.first?.arguments == ["log", "--format=%H%x1f%at%x1f%s%x1f%an"])
    }

    @Test("log parses multiple commits into ordered entries")
    func logParsesMultipleCommits() async throws {
        let runner = MockProcessRunner()
        let output = [
            "abc123\u{1F}1755000000\u{1F}autosave — 1 file, +50 words\u{1F}Josiah Massari",
            "def456\u{1F}1754900000\u{1F}checkpoint\u{1F}Josiah Massari"
        ].joined(separator: "\n") + "\n"
